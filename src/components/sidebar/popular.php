            ...
            <div class="relative flex space-x-3 rounded transition">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex flex-col focus:outline-none gap-1">
                        <a href="/komik/<?php echo $manga['endpoint']; ?>"
                            class="verflow-hidden truncate w-full max-w-[160px] md:max-w-[220px] text-sm font-medium transition hover:text-primary-600 truncate">
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex">
                                <?php
                                for ($i = 0; $i < 5; $i++) {
                                    $class = $i < ($manga['rating'] / 2) ? 'text-accent-500' : 'text-primary-300';
                                    echo "<i class='fas fa-star $class'></i>";
                                }
                                ?>
                            </div>
                            <p class="font-medium text-sm text-primary-400"><?php echo $manga['rating']; ?></p>
                        </div>
                        <p
                            class="flex flex-wrap font-medium text-xs gap-[5px] overflow-hidden truncate max-h-[60px] leading-[10px]">
                            Genres :
                            <?php foreach ($manga['genres'] as $genre): ?>
                                <a href="/genre/<?php echo $genre['endpoint']; ?>"
                                    class="text-primary-600 text-[0.75rem] transition hover:text-white"><?php echo $genre['label']; ?></a>
                            <?php endforeach; ?>
                        </p>
                    </div>
                </div>
